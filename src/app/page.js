  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between' }}>
        ...
        <span style={{ fontSize:10, color:'#475569', fontFamily:'monospace' }}>{zone}</span>
...
function AIConclusion({ symbol, name, data, returnPct, isWatchlist }) {
  ...
  useEffect(() => {
    ...
    const context = isWatchlist
      ? `Dit is een WATCHLIST positie — je bezit het nog niet.`
...
- Volatiliteit: ${data.vol ?? '—'}%
- ${context}

Zin 1: Wat zeggen de technische metrics?
Zin 2: ${isWatchlist ? 'Is dit nu een goed instapmoment? Waarom wel/niet?' : 'Bijhouden/bijkopen/afbouwen/verkopen met reden.'}
  ...
  return (
    <div style={{ background:'#0a1628', border:'1px solid #1e3a5f', borderRadius:8, padding:'12px 14px', marginTop:12 }}>
      <div style={{ fontSize:9, color:'#3b82f6', textTransform:'uppercase', letterSpacing:'0.1em', fontFamily:'monospace', marginBottom:8 }}>
        🤖 AI Conclusie {isWatchlist ? '— Instapmoment?' : ''}
      </div>
      {loading
        ? <div style={{ color:'#334155', fontSize:12, fontFamily:'monospace' }}>Analyseren...</div>
...
function StockCard({ row, prices, isWatchlist, expanded, onToggle }) {
  ...
  const returnPct = isWatchlist ? null : (() => {
    ...
    if (!buyPrice || !currentPrice) return parseFloat(row['Rendement %']) || null;
          ...
          <div style={{ textAlign:'right', flexShrink:0 }}>
            {d.price && (
              <div style={{ fontFamily:'monospace', fontSize:13, color:'#cbd5e1', fontWeight:500 }}>
                {d.price.toFixed(2)} {row.Currency}
              </div>
            )}
            {returnPct != null && (
              <div style={{ fontFamily:'monospace', fontSize:12, fontWeight:700, color:pc(returnPct), marginTop:2 }}>
                {ps(returnPct)}{returnPct}%
              </div>
            )}
          </div>
        </div>
        <div style={{ marginBottom:10 }}><SignalBadge signal={signal} /></div>
...
export default function Page() {
  ...
  const filterFn = row => !filterSig || (prices[row.Symbol]||{}).signal === filterSig;

  const activeRows = (tab === 'portfolio' ? portfolio : watchlist)
    .filter(filterFn).sort(sortFn);

  const sigCounts = {};
  (tab==='portfolio' ? portfolio : watchlist).forEach(r => {
    const sig = (prices[r.Symbol]||{}).signal;
    if (sig) sigCounts[sig] = (sigCounts[sig]||0)+1;
  });

  const winners = portfolio.filter(r => {
    const d = prices[r.Symbol]||{};
    const bp = parseFloat(r['Buy Price']);
    return bp && d.price && d.price > bp;
  }).length;
  ...
  const tabBtn = active => ({
    flex:1, padding:'10px', background: active?'#1e3a5f':'transparent',
    color: active?'#60a5fa':'#64748b', border:'none',
  ...
  return (
    <div style={{ fontFamily:"'SF Pro Display',-apple-system,sans-serif", background:'#0a0e1a', minHeight:'100vh', color:'#e2e8f0', paddingBottom:40 }}>
      ...
      <div style={{ background:'linear-gradient(180deg,#111827,#0a0e1a)', padding:'52px 20px 16px', borderBottom:'1px solid #1e293b' }}>
        ...
        <div style={{ fontSize:32, fontWeight:700, letterSpacing:'-0.02em', color:'#f1f5f9' }}>
          {portfolio.length > 0 ? `${portfolio.length} posities` : 'Laden...'}
        </div>
        <div style={{ fontSize:12, color:'#475569', fontFamily:'monospace', marginTop:4 }}>
          Live via Google Sheets · {updated ? updated.toLocaleTimeString('nl-BE',{hour:'2-digit',minute:'2-digit'}) : '—'}
        </div>

        <div style={{ display:'flex', gap:10, marginTop:14, flexWrap:'wrap' }}>
          {[
            { label:'Portfolio',  value:portfolio.length,  color:'#f1f5f9' },
            { label:'Watchlist',  value:watchlist.length,  color:'#60a5fa' },
            { label:'Winnaars',   value:winners,            color:'#22c55e' },
            { label:'Verliezers', value:portfolio.length-winners, color:'#ef4444' },
          ].map(s => (
      ...
      <div style={{ display:'flex', borderBottom:'1px solid #1e293b', background:'#0f172a' }}>
        <button onClick={() => { setTab('portfolio'); setFilterSig(null); }} style={tabBtn(tab==='portfolio')}>
          📊 Portfolio ({portfolio.length})
        </button>
        <button onClick={() => { setTab('watchlist'); setFilterSig(null); }} style={tabBtn(tab==='watchlist')}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 16px', gap:8, overflowX:'auto' }}>
        ...
        <button onClick={fetchData} disabled={loading} style={{ ...btn(false), color:loading?'#334155':'#93c5fd', flexShrink:0 }}>
          {loading?'...':'⟳'}
        </button>
      </div>
      ...
      {loading && activeRows.length === 0 ? (
        <div style={{ textAlign:'center', padding:60, color:'#334155', fontFamily:'monospace', fontSize:13 }}>
          <div style={{ animation:'pulse 1.5s infinite' }}>Laden via Google Sheets...</div>
        </div>
      ) : activeRows.length === 0 ? (
        <div style={{ textAlign:'center', padding:60, color:'#334155', fontFamily:'monospace', fontSize:13 }}>
          {tab==='watchlist' ? 'Voeg aandelen toe aan je Watchlist tabblad in Google Sheets' : 'Geen posities gevonden'}
        </div>
      ) : (
        activeRows.map(row => (
          <StockCard
      ...
      <div style={{ textAlign:'center', padding:'20px 0 0', fontFamily:'monospace', fontSize:10, color:'#1e293b' }}>
        Data via Yahoo Finance · Google Sheets · RSI-14 · MA50/200 · Geen beleggingsadvies
      </div>
    </div>
  );
}
